    ...
    def test_create_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                resp = client.post('/item/test', data={'price': 42, 'store_id': 1})
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                resp = client.put('/item/test', data={'price': 42, 'store_id': 1})

                self.assertEqual(resp.status_code, 200)
                self.assertEqual(ItemModel.find_by_name('test').price, 42)
                self.assertDictEqual(json.loads(resp.data),
                                     {'name' : 'test', 'price' : 42})
